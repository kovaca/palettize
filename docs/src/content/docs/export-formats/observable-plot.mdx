...
| `type` | `str` | (unset) | Plot scale type. When unset, follows `--scale` (`power` becomes `pow`). `symlog` is not a Plot type and falls back to `linear` with a warning |
