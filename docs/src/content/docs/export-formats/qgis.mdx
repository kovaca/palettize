...
| `ramp_type` | `str` | `"gradient"` | `"gradient"`, `"exact"`, or `"approximate"`. Only `gradient` uses `--scale` |
| `name` | `str` | (unset) | Ramp name. Defaults to the colormap name |
| `tags` | `list` | (unset) | Comma-separated tags. On the CLI: `-O tags=scientific,perceptual` |
| `opacity` | `float` | `1.0` | Overall opacity (0.0-1.0) |
| `color_space` | `str` | (unset) | QGIS `colorSpace` attribute hint, e.g. `RGB` |
