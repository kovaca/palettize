...
`--category` is one of `sequential`, `diverging`, `cyclic`, `qualitative`, or `miscellaneous`. `--namespace` filters by publisher, and `--swatch` draws a preview. See [`palettize list`](/palettize/cli/list/).
