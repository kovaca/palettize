...
class GdalExporter(BaseExporter):
    ...
    def export(
        ...
        Accepted options:
            ...
            verbose (bool): Include comment header lines. Default False.
            nodata (bool): Include nv line by default. Default True.
        """
        options = options or {}
        ...
        # Handle verbose option (may be string from CLI)
        verbose = options.get("verbose", False)
        if isinstance(verbose, str):
            verbose = verbose.lower() in ("true", "1", "yes")

        # Handle nodata option (may be string from CLI)
        nodata = options.get("nodata", True)
        if isinstance(nodata, str):
            nodata = nodata.lower() in ("true", "1", "yes")
...
def _export_gdal_color_relief_impl(  # Renamed original function
    ...
    # Include comments only when verbose mode is enabled
    if verbose:
    ...
    # If nodata=True and no explicit nodata_value, use default "nv"
    if nodata and nodata_value is None:
        nodata_value = "nv"
    ...
    for i in range(num_colors):
        ...
        try:
            ...
            if not (isinstance(color_components, tuple) and len(color_components) == 4):
                ...
                logger.warning(
                    "Color for data_val %s not in expected RGBA tuple format. Got: %s. Skipping.",
                    data_val,
                    color_components,
                )
                continue
            ...
            logger.warning(
                "Could not generate color for data_val %s (normalized_pos from scaler). Error: %s",
                data_val,
                e,
            )
